...
def get_suggestion():
  url = 'http://135.12.153.5:8000/suggest'
...
async def on_message(message):
  ...
  if msg == (prefix + 'mxar'):
    ...
    embedMembers.add_field(name = "Bday", value = "He didn't tell me in time", inline = False)
    await message.channel.send(embed = embedMembers)
  ...
  #---using lovelance---
  if msg.lower() == (prefix + "topic"):
    embed = discord.Embed(title = "Random Conversation Topic", description = str(get_suggestion()).replace("Random conversation topic: ", ""), color = discord.Color.red())
    embed.set_footer(text = "*Using Pascal AI created by Jenny :D*")
    await message.channel.send(embed = embed)
    #await message.channel.send(get_suggestion())


  if message.content.startswith(prefix + "chat"):
    #await message.channel.send("The Ai is acting lil sus, it'll be back soon -GT")
    channel = message.channel.id
    if channel != 243035352187666448:
      user_input = message.content[len(prefix + "chat"):].strip()
      
      #chat_api_url = f"http://135.12.153.5:8000/infer?prompt={user_input}&user_name={message.author.name}"
      #lovelace_response = requests.get(chat_api_url)

      geminiResponse = getGeminiResponse(prompt=user_input)
      await message.channel.send(geminiResponse)
      
      '''if lovelace_response.status_code == 200:
        data = lovelace_response.json()
        output = data['output']
        await message.channel.send(output)
      else:
        await message.channel.send("lovelance bot failed to respond")'''
      
    else:
      await message.channel.send("Please use this in Bot Abuse only")
  ...
  if message.content.startswith(prefix + "support"):
    ...
    user_timers[user_name] = int(50)
    
    if channel != 243035352187666448:
      user_input = message.content[len(prefix + "support"):].strip()
      support_url = f'http://135.12.153.5:8000/support?question={message.content}&user_name={message.author.name}'
      lovelace_response = requests.get(support_url)
      ...
      if lovelace_response.status_code == 200:
        ...
        '''while user_timers[user_name] != 0:
          time.sleep(1)
          user_timers[user_name] -= 1
          
        endticket = f"http://135.12.153.5:8000/endticket?user_name={message.author.name}"
        end = requests.get(endticket)
        if end.status_code == 200:
          end_data = end.json()
          end_output = end_data['output']
          print(end_output)
          await message.channel.send(end_output)'''
  ...
  if message.content.startswith(prefix + "endticket"):
    endticket = f"http://135.12.153.5:8000/endticket?user_name={message.author.name}"
    end = requests.get(endticket)
    if end.status_code == 200:
      end_data = end.json()
  ...
  if message.content.startswith(prefix + "forget"):
    forget_url = f"http://135.12.153.5:8000/forget?user_name={message.author.name}"
    forget = requests.get(forget_url)
    if forget.status_code == 200:
      forget_data = forget.json()
      forget_output = forget_data['output']
      print(forget_output)
      await message.channel.send(forget_output)
          
  if message.content.startswith(prefix + "stop"):
      stop_url = f"http://135.12.153.5:8000/stop?user_name={message.author.name}"
      stop = requests.get(stop_url)
      if stop.status_code == 200:
        stop_data = stop.json()
        stop_output = stop_data['output']
        print(stop_output)
        await message.channel.send(stop_output)
  ...
  if msg.lower() == (prefix + 'help'):
    ...
    embed.add_field(name = "%topic", value = "Uses AI to generate a topic for everyone to talk about", inline = False)
    embed.add_field(name = "%chat <your message>",value = "Talk with the AI created by Jenny :D\nOnly Works In <#710259442318442518>", inline = False)
